...
static void luabind_<%= class:get_bind_name() %>(lua::state& l) {
    ...
    <%= method:get_bind('async') and 'llae::async_function' or 'lua::bind::function' %>(l,"<%= method:get_lua_name() %>",&<%= method:get_c_ptr() %><% if method:get_policy() then %>,lua::bind::<%= method:get_policy() %><% end %>);<% if method:get_bind('alias') then %>
    lua::bind::function(l,"<%= method:get_bind('alias') %>",&<%= method:get_c_ptr() %>);<% end %><% end %>
