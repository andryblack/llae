...
function _M.lib( root )
	...
	os.mkdir(path.join(root,'build','bootstrap','include'))
	os.mkdir(path.join(root,'build','bootstrap','include','llae-private'))
	...
	utils.preprocess(
		...
		path.join(root,'build','bootstrap','include','llae-private','luaconf.h'),
		{replace={
			['LUA_API'] = '',
			['LUAI_FUNC'] = ''
		}})
